...
export async function POST(request: Request) {
    ...
    // Note: Use the updated API version compatible with the installed library
    const stripe = new Stripe(process.env.STRIPE_SECRET_KEY as string, {
        apiVersion: '2025-12-15.clover', // Using the correct API version
        typescript: true,
    });
    ...
    try {
        ...
        return new Response(JSON.stringify({
            ...
            paymentIntent: paymentIntent.client_secret, // Retain this for legacy frontend support if needed
            publishableKey: process.env.EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY, // Requested by frontend
            amount: amountMinor,
            currency: "qar",
        }), {
            status: 200,
